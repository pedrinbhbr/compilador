...
void AnalisadorSintatico::ATRIBUICAO()
{
    ...
    if (sID.tipo == TIPO_FLOAT)
    { // tipo real
        if (rsEXP.tipo == TIPO_INT)
        { // id real recebendo EXP inteira
            // Converte em real.
    ...
    else if (sID.tipo == TIPO_INT)
    { // id inteiro recebe apenas EXP inteiro
    ...
    { // id char recebe apenas EXP char
    ...
    { // id tipo string
        if (stringArranjo)
        { // id string recebendo caractere em alguma posicao da string
...
void AnalisadorSintatico::EXP(RegistroSemantico *rsEXP)
{
    ...
    case TOKEN_IGUAL:
        ...
        {
            if (rsEXPS_aux.tipo == TIPO_INT)
            { // verifica tipo de EXPS
            ...
            if (rsEXPS1.tipo == TIPO_INT)
            { // verifica tipo de EXPS1
...
void AnalisadorSintatico::EXPS(RegistroSemantico *rsEXPS)
{
           ...
           analisadorLexico.getToken() == TOKEN_OR)
    {
        ...
        else if (rsT.tipo == TIPO_BOOL && rsT1.tipo != TIPO_BOOL)
        {                                                         // para OR ambos devem ser logicos
            verificaErroTipo(analisadorLexico.num_linhas, 1, ""); // tipo incompativel
        }                                                         // fim if else
        ...
        if (tokenNegativo || tokenPositivo)
        { // se for operacao aritmetica: subtracao ou soma
            if (rsT.tipo == TIPO_INT)
            { // se resultado eh inteiro, ambos T e T1 eram inteiros
            ...
            { // se resultado eh real, pelo menos um deles eh real
                // Guarda valor de T em um registrador
                if (rsT_auxTipo == TIPO_INT)
                { // valor de T eh inteiro
                    codigoAssembly.append("\tmov eax,[qword M+" + EnderecoHex(rsT.endereco) + "]\t\t; Carrega conteudo de T.end no eax\n");
                    // Converte em real.
                    codigoAssembly.append("\tcvtsi2ss xmm0,eax\t\t; converte reg para real\n");
                }
                ...
                if (rsT1.tipo == TIPO_INT)
                { // valor de T1 eh inteiro
...
void AnalisadorSintatico::T(RegistroSemantico *rsT)
{
           ...
           analisadorLexico.getToken() == TOKEN_MOD)
    {
        ...
        case TOKEN_MULTIPLICACAO:
            ...
            if (rsF.tipo != TIPO_INT && rsF.tipo != TIPO_FLOAT)
            {                                                         // precisa ser tipo numerico
                verificaErroTipo(analisadorLexico.num_linhas, 1, ""); // tipo incompativel
            }                                                         // fim if
        ...
        case TOKEN_AND:
            ...
            if (rsF.tipo != TIPO_BOOL)
            {                                                         // precisa ser tipo logico
                verificaErroTipo(analisadorLexico.num_linhas, 1, ""); // tipo incompativel
            }                                                         // fim if
            tokenAnd = true;
            break;
        ...
        case TOKEN_DIV:
            ...
            {                                                         // precisa ser tipo numerico inteiro
                verificaErroTipo(analisadorLexico.num_linhas, 1, ""); // tipo incompativel
            }                                                         // fim if
            tokenDiv = true;
            break;
        ...
        if (tokenDivisao || tokenMultiplicacao)
        { // verifica se resultado final eh inteiro ou real
            if (rsF1.tipo != TIPO_INT && rsF1.tipo != TIPO_FLOAT)
            {                                                         // precisa ser tipo numerico
                verificaErroTipo(analisadorLexico.num_linhas, 1, ""); // tipo incompativel
            }
            else if (tokenMultiplicacao)
        ...
        else if (rsF.tipo == TIPO_INT && rsF1.tipo != TIPO_INT)
        {                                                         // realiza div ou mod e so aceita tipos inteiros. resultado inteiro
            verificaErroTipo(analisadorLexico.num_linhas, 1, ""); // tipo incompativel
        }
        else if (rsF.tipo == TIPO_BOOL && rsF1.tipo != TIPO_BOOL)
        {                                                         // realiza AND binario e so aceita tipos logicos. resultado logico
            verificaErroTipo(analisadorLexico.num_linhas, 1, ""); // tipo incompativel
        }                                                         // fim if else
...
void AnalisadorSintatico::F(RegistroSemantico *rsF)
{
    ...
    else if (analisadorLexico.getToken() == TOKEN_INT || analisadorLexico.getToken() == TOKEN_FLOAT)
    {
        ...
        { // conversao para real
            if (rsEXP.tipo == TIPO_INT)
            { // EXP inteira. Precisa de conversao
